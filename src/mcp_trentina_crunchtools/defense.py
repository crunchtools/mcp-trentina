...
async def defend(
    ...
    *,
    ...
    if defense is None or defense.classify:
        if guarded:
    ...
    # Precedence: L2, then L3, then L1. Matches every hand-rolled copy and is
    # pinned by tests/test_defense_characterization.py.
    flagged_by: Layer | None = None
    risk_level = l1_risk
    assessment: dict[str, Any] | None = None

    if l2_flagged and classification is not None:
        flagged_by = Layer.L2
        risk_level = "high"
        assessment = _l2_assessment(classification)
    elif l3_flagged and l3_assessment is not None:
        flagged_by = Layer.L3
        risk_level = str(l3_assessment.get("risk_level", "high"))
        assessment = l3_assessment
    elif l1_flagged:
        flagged_by = Layer.L1
        risk_level = l1_risk
