...
Trentina is a secure gateway that inspects everything between your AI agents and the outside world — web content, MCP tool names, descriptions, and responses, Matrix messages, LLM completions, and monitoring alerts. It has two benefits: security and cutting token costs. For security, everything goes through a [three-layer defense pipeline](docs/defense-pipeline.md) at every ingress, with per-profile enforcement (block, redact, flag) and a full audit trail. All content which reaches your agent's LLM is guaranteed to flwo through the three-layer defense pipleine: what your agent reads is always judged with Trentina's verdict, even in flag mode (useful for security research). To reduce token usage, noise in MCP tool names, descriptions and responses are minimized by default leaving more context for doing real work, and giving the agent stronger signal to operate on. Named after the 1377 quarantine system from Ragusa, where incoming ships had to anchor offshore for thirty days before anyone was allowed into the city. Same idea: keep the commerce flowing without letting something dangerous through.
